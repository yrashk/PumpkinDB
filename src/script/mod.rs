...
pub struct Env<'a> {
    ...
    // current TRY status
    tracking_errors: bool,
    aborting_try: Option<Error>
}
...
impl<'a> VM<'a> {
    ...
    fn handle_try_end(&mut self, mut env: Env<'a>, word: &'a [u8], _: EnvId) -> PassResult<'a> {
        ...
        env.tracking_errors = false;
        if env.aborting_try.is_none() {
            env.push(_EMPTY);
            Ok((env, None))
        } else if let Some(Error::ProgramError(err)) = mem::replace(&mut env.aborting_try, None) {
