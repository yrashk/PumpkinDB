...
impl Server {
            ...
            loop {
                match receiver.recv() {
                    Ok((topic, message, callback)) => {
                        if topic == "subscriptions".as_bytes() {
                            ...
                            let _ = callback.send(());
                            if map.contains_key(&topic) {
                                for token in map.get(&topic).unwrap() {
                                    let _ = evented_sender.send((*token, topic.clone(), message.clone()));
                                }
                            }
                        }
                    },
    ...
    fn readable(&mut self, token: Token) -> io::Result<()> {
        while let Some(mut message) = self.find_connection_by_token(token).readable()? {
            ...
            let id = EnvId::new();
            let mut vec = Vec::new();
            let mut prefix = parse(format!("[{} 2 WRAP \"subscriptions\" SEND] 'SUBSCRIBE DEF", token.0).as_str()).unwrap();
